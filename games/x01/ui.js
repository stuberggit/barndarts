...
function renderSetup(container, state) {
  container.innerHTML = `
    ...
    <div id="x01Options"></div>
    <div id="modal"></div>
  `;

  const options = document.getElementById("x01Options");
  options.style = `
    display:grid;
  ...
  [101, 201, 301, 401, 501, 601, 701, 801, 901, 1001].forEach(score => {
    const btn = document.createElement("div");
    btn.innerText = String(score);
    btn.style = `
      ${buttonStyle()}
      padding:14px;
      min-height:52px;
        ...
        throws.length === 0
          ...
          : throws.map((throwRecord, index) => `
            ...
            ">
              ...
              <div style="color:#facc15;">-${getThrowValue(throwRecord)}</div>
            </div>
          `).join("")
      }
    </div>
